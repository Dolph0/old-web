...
#
# Selling the code for this script, without prior written consent of the
...
# notify the author and verify the licensing terms.
#
...
function IESplit(re, st) {
    ...
    while (re.test(st)) {
        x = re.exec(st);
        st = RegExp.rightContext;
        ret.push(RegExp.leftContext);
        ret.push(RegExp.lastMatch);
    }
    return ret;
}
...
Calendar.DOW = ["Domingo", "Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado"];
Calendar.count = 0;
...
function Calendar_processDateData(vInputData, pFormat) {
    ...
    if ((m = vInputData.match(ree)) != null) {
        ...
        var mach = pFormat.split(new RegExp(",", "g"))
        var i = 0;
        for (i=0; i<mach.length; i++){
                if (Calendar.readDate(vInputData, mach[i]))
                   return Calendar.readDate(vInputData, mach[i]);
        }
        return null;
    }
}
...
Calendar.prototype.getMonthlyCalendarCode = function() {
    ...
    vCode = vCode + "<TABLE WIDTH='" + (v_Width-50) + "' BORDER=1 BGCOLOR=\"" + this.gBGColor + "\">";
...
Calendar.prototype.show = function() {
    ...
    this.wwrite("<link rel=stylesheet href='<? cheqroot('comun/estilo_webmogan.css', TRUE);?>'>");
    if (this.gType == "POPUP")
    ...
    this.wwrite("<body " + 
            ...
            "text=\"" + this.gTextColor + "\" LANGUAGE=javascript onload=\"window.focus();\" onBlur=\"window.focus();\">");
    
    // For Smart navigation
    if (v_SmartNav == "Yes")
        this.wwrite("<form name=\"frmCal\" onsubmit=\"return false;\">");

/*    this.wwrite("<TABLE class='izqform' WIDTH='" + (v_Width-50) + "' BORDER=1 CELLPADDING=0><TR><TD>" +
        "<TABLE class='izqform' BORDER=0 WIDTH='100%' CELLPADDING=1>" +
        "<TR><TD>" +
    ...
    this.wwrite("<TABLE class='izqform' WIDTH='" + (v_Width-50) + "' BORDER=1 CELLPADDING=0><TR><TD>" +
        "<TABLE class='izqform' BORDER=0 WIDTH='100%' CELLPADDING=1>" +
        "<TR><TD class='calendtitleform'>" +
        "<B>" + this.gTitle + "</B>" +
        "</TD><TD class='calendtitleform' ALIGN=RIGHT>" +
        "<B>" +
        "<A HREF='javascript:" +
        this.WHO + "Calendar.Close(\"" + this.gType + "\", \"" + this.INLINE + "\");' " +
        "STYLE='color:white'>" + 
    ...
    this.wwrite("<TD class='calendmonthform'>");

/*    this.wwrite("<TABLE class='derform' WIDTH='" + (v_Width-30) + "' BORDER=0 CELLPADDING=0 BGCOLOR='#000099'><TR><TD>" +
        "<TABLE class='derform' BORDER=0 WIDTH='100%' CELLPADDING=1 BGCOLOR='#B7B7C7'>" +
    ...
    this.wwrite("<TD class='derform' BGCOLOR='#B7B7C7'>");*/
    this.wwriteA("<B>");
...
//BGCOLOR='#e0e0e0'
    if (v_Nav == "Yes") {
        this.wwrite("<TABLE class='calendform' WIDTH='" + (v_Width-50) + "' BORDER=0 CELLSPACING=0 CELLPADDING=0 ><TR><TD>");
...
Calendar.prototype.showY = function() {
    ...
    this.wwrite("<TABLE WIDTH='" + (v_Width-50) + "' BORDER=1 CELLSPACING=0 CELLPADDING=0 BGCOLOR='#e0e0e0'><TR><TD ALIGN=center>");
    this.wwrite("[<A HREF=\"" +
...
Calendar.prototype.cal_header = function() {
    ...
    for (i=v_WeekStart,j=0; j<7; i++,j++,i=i%7)
        vCode = vCode + "<TD class='calenddaysform' WIDTH='" + ((i==6) ? 16 : 14) + "%'>" +
            "<B>" +
            Calendar.DOW[i].substring(0, 3) + "</B></TD>";
    vCode = vCode + "</TR>";
...
Calendar.prototype.cal_data = function() {
    ...
    */
    ...
    for (j=vFirstDay; i<7; j++,i++,j=j%7) {
        ...
        vCode = vCode + "<TD class='calendform' WIDTH='14%'" + this.write_weekend_string(j) +
                ">" +
                linkText + 
    ...
    for (m=1; m<(7-i); m++) {
        if (this.gYearly)
            vCode = vCode + "<TD class='calendform' WIDTH='14%'" + this.write_weekend_string(j+m) +
...
Calendar.prototype.format_data = function(p_day) {
    ...
    var mach = this.gFormat.split(new RegExp(",", "g"))
    var vData = mach[0];
...
function Calendar_CreateCalendarLayer(pLeft, pTop, pInitText) {
    ...
    */
    ...
    if (isIE)
        ...
        "position:absolute;top:" + pTop + ";left:" + pLeft + ";z-index:'65535';" +
        "\">" + pInitText + "<\/DIV>");
    else if (isDOM)
        document.writeln("<DIV ID=\"" + Calendar_ID + "\" STYLE=\"" + 
        "position:absolute;top:" + pTop + ";left:" + pLeft + ";z-index:'65535';" +
...
function Build(p_item, p_month, p_year, p_format, p_type, p_custom) {
    ...
    v_WeekStart = chkVar("v_WeekStart") ? v_WeekStart : 0;

    var vHeight, vWidth;
    v_Width = chkVar("v_Width") ? v_Width : 300;
    v_Height = chkVar("v_Height") ? v_Height : 230;

    if (p_type == "POPUP") {
        vWidth = chkVar("v_Width") ? v_Width : 300;

        if (isIE) vHeight = chkVar("v_Height") ? v_Height : 230;
        ...
        if (v_PopupX >= 0 && v_PopupY >= 0) {
            if (!ggWinCal || ggWinCal.closed)
                ggWinCal = window.open("", "Calendar", 
                    ...
                    ",titlebar=no,status=no,resizable=" + v_Resizable +
