<?php
include "func_webmogan.inc";
include "loadGETPOST.inc";
...
// variable nombvias, por lo que no hay que acceder a la base de datos para obtener la sigla
// Obtengo el listado de inmuebles de esa via

$query= "SELECT nomb, codiinmu, codiviax, nume, letr, esca, plan, puer, refecata, 
         numecarg, caracont, nfij, idenloca FROM webmogan_inmu inmu WHERE codiayun = '$ayun' ";
if ( $codiviax != "" ) {
...
// Probando...
if ( $tipoobje == 'busq' ) {
  $query2 = "SELECT distinct refecata FROM webmogan_inmu inmu WHERE codiayun = '$ayun' ".$where;
  $resp2 = sql( $query2 );
  if ( $resp2 ) {
 ...
 if (!$nombviax) {
   $nombviax=sql ("SELECT siglvias.nomb||' '||vias.nomb
                     FROM (webmogan_vias vias INNER JOIN webmogan_inmu inmu ON inmu.codiviax=vias.codiviax)
...
if ( is_array( $resp) ) {
  if ( $contvias > 1 ) {
      ...
      print "<center><table cellpadding=4 cellspacing=1 border=1 width=80%>\n";
      print "<tr>\n<td align=\"center\" width=\"10%\" rowspan=\"3\"><img src=\"../imag/recanet.gif\" width=\"55\" height=\"48\" border=\"0\"></td>\n";
      ...
      print "<tr>";
      print "<td class='izqform' width=5%>N&#176;</td>\n";
  ...
  // En otro caso, tenemos que poner los valores en las cajas. 
           ...
           echo "long = opener.document.form.codiviax$sufi.length;                                                    for (i=1; i<long; i++) {
			            ...
			            // opener.document.getElementById(\"BotonMapa\").style.visibility=\"visible\";
                                    ";
        ...
        print "<td class='derform'>&nbsp;{$camp['puer']}";
