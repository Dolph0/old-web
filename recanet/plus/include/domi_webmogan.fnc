...
function combosuje ($valor)
{
  $retornar="
             <select class='inputform' name='_idxxdecl' size='1'  onChange=\"datodecl()\">
                ...
                <option value = 'RT' ";if ($valor=='RT')$retornar.=" selected";$retornar.=">REPRESENTANTE DEL ADQUIRENTE</option>
                <option value = 'RA' ";if ($valor=='RA')$retornar.=" selected";$retornar.=">REPRESENTANTE DEL TRANSMITENTE</option>
             </select>";
  return $retornar;
       ...
       <table cellpadding=4 cellspacing=1 border=0>
        ...
        <tr>
          <td id="celdpaisdato">
            <? if (!${$de."codipais"}) {
		   	  ...
		   	  print("<input type='hidden' name='".$de."codipais' value='$codiespa'>");
            ?>
          </td>
        </tr>
          <tr>
        ...
        <tr id="<?echo $de."filaprovmunidato"?>" <? print (${$de."codipais"} == $codiespa)?'':" style='display:none'"; ?> >
          <td colspan=2><? mueslist ($de."prov", "SELECT codiprov, nomb FROM prov ORDER BY nomb", ${$de."prov"}, "cambiamuni(form.".$de."prov, form.".$de."muni)", 1) ?></td>
            <td>
            <? 
            mueslist ($de."muni", "SELECT codimuni, nomb FROM muni WHERE codiprov = '${$de.'prov'}' ORDER BY nomb", ${$de."muni"}, "", 1) ?>
...
if ($_clastran=='CVT' || $_clastran=='PER')
 {
  if ($de=="tran") $codifigu = sql("SELECT codifigu FROM tercfigu WHERE descfigu = 'CONTRIBUYENTE PRINCIPAL'");
  if ($de=="adqu") $codifigu = sql("SELECT codifigu FROM tercfigu WHERE descfigu = 'ADQUIRENTE OIPL'");
 } else {  
  if ($de=="tran") $codifigu = sql("SELECT codifigu FROM tercfigu WHERE descfigu = 'ADQUIRENTE OIPL'");
  if ($de=="adqu") $codifigu = sql("SELECT codifigu FROM tercfigu WHERE descfigu = 'CONTRIBUYENTE PRINCIPAL'");
 }

if ($de=="repr")  $codifigu = sql("SELECT codifigu FROM tercfigu WHERE descfigu = 'DECLARANTE OIPL'");
...
if (${$de."nomb"} && ${$de."nifx"} && $sesi[sesicodiusua])
 ...
 {
   if (!${$de."nomb"})       $ERROR_SQL.="Falta el nombre $desujeto<br>";
   if (!${$de."nifx"})       $ERROR_SQL.="Falta el nif $desujeto<br>";
   if (!$sesi[sesicodiusua]) $ERROR_SQL.="Se desconoce el usuario que grabara los datos<br>";
 }
