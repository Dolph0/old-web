...
class RedsysAPI{
	...
	function stringToArray($datosDecod){
		if (PHP_VERSION_ID < 50200) {
                        // PARCHE (Alexis 06/11/2015)
                        // Resulta que obtenemos un OBJETO y tengo
                        // que devolver un array. 
			$vars_pay_aux = $this->json_decode4($datosDecod);
			$this->vars_pay = toArray($vars_pay_aux);
		} else {
